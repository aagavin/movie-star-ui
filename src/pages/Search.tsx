import { IonButtons, IonMenuButton, IonContent, IonHeader, IonPage, IonSearchbar, IonTitle, IonToolbar } from "@ionic/react";
...
const Search = (): JSX.Element => {
  ...
  const formatResults = (results: any) => {
    ...
    return results.filter((r: any) => r.id.startsWith('tt')).map((r: any) => ({
      ...r,
      title: r.l,
      title_cast: r.s,
      poster: r?.i?.imageUrl
  ...
  const search = async () => {
    ...
    return fetch(url, {
      ...
      method: 'GET',
    }).then((r) => r.json()).then(r => r?.d).then(formatResults);
  }
